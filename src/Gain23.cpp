...
GainType Gain23(void) {
  ...
  for (X2 = 1; X2 <= 2; X2++) {
    ...
    do {
      ...
      for (const auto& c2 : s2->candidates) {
        Ns2 = c2;
        s3 = Ns2->To;
        if (s3 == s2->Prd || s3 == s2->Suc) continue;
        if (++Breadth2 > param.max_breadth) break;
        G1 = G0 - Ns2->Cost;
        for (X4 = 1; X4 <= 2; X4++) {
          s4 = X4 == 1 ? SUC(s3) : PRD(s3);
          G2 = G1 + context.C(s3, s4);
            ...
            for (X6 = 1; X6 <= 2; X6++) {
              if (X4 == 2) {
                if (X6 == 1) {
                  ...
                  s6 = Case6 == 1 ? SUC(s5) : PRD(s5);
                } else {
                  s6 = s6 == s5->Prd ? s5->SucNode() : s5->PrdNode();
                  if (s5 == s1 || s6 == s1) continue;
                  Case6 += 2;
                }
              } else if (BETWEEN(s2, s5, s3)) {
                Case6 = 4 + X6;
                s6 = X6 == 1 ? SUC(s5) : PRD(s5);
                if (s6 == s1) continue;
              } else {
                if (X6 == 2) break;
                Case6 = 7;
              ...
              G4 = G3 + context.C(s5, s6);
              Gain6 = 0;
              if (s6 != s1 && (Gain6 = G4 - context.C(s6, s1)) > 0) {
                if (Gain6 > 0) {
                  if (Case6 <= 2 || Case6 == 5 || Case6 == 6) {
                    Make3OptMove(s1, s2, s3, s4, s5, s6, Case6);
                    if (Improvement(&Gain6, s1, s2)) return Gain6;
                  }
                }
                Gain = BridgeGain(s1, s2, s3, s4, s5, s6, 0, 0, Case6, Gain6);
                if (Gain > 0) return Gain;
              }
              Breadth6 = 0;
              /* Choose (s6,s7) as a candidate edge
                 emanating from s6 */
              for (const auto& c6 : s6->candidates) {
                Ns6 = c6;
                s7 = Ns6->To;
                if (s7 == s6->Prd || s7 == s6->Suc || (s6 == s2 && s7 == s3) ||
                    (s6 == s3 && s7 == s2) || (G5 = G4 - Ns6->Cost) <= 0)
                  continue;
                ...
                for (X8 = 1; X8 <= 2; X8++) {
                  if (X8 == 1) {
                    ...
                    switch (Case6) {
                      case 1:
                        s8 = BETWEEN(s2, s7, s5) ? SUC(s7) : PRD(s7);
                        break;
                      case 2:
                        s8 = BETWEEN(s3, s7, s6) ? SUC(s7) : PRD(s7);
                        break;
                      case 3:
                        if (BETWEEN(s5, s7, s4))
                          s8 = SUC(s7);
                        else {
                          s8 = BETWEEN(s3, s7, s1) ? PRD(s7) : SUC(s7);
                          Case8 = 17;
                        }
                        break;
                      case 4:
                        if (BETWEEN(s2, s7, s5))
                          s8 = BETWEEN(s2, s7, s4) ? SUC(s7) : PRD(s7);
                        else {
                          s8 = PRD(s7);
                          Case8 = 18;
                        }
                      ...
                      case 5:
                        ...
                        break;
                      case 6:
                        s8 = BETWEEN(s2, s7, s3) ? SUC(s7) : PRD(s7);
                        break;
                      case 7:
                        if (BETWEEN(s2, s7, s3))
                          s8 = SUC(s7);
                        else {
                          s8 = BETWEEN(s5, s7, s1) ? PRD(s7) : SUC(s7);
                          Case8 = 19;
                        }
                    }
                  } else {
                    if (Case8 >= 17 || (Case6 != 3 && Case6 != 4 && Case6 != 7))
                      break;
                    s8 = s8 == s7->Prd ? s7->SucNode() : s7->PrdNode();
                    Case8 += 8;
                  }
                  if (s8 == s1 || (s7 == s1 && s8 == s2) ||
                      (s7 == s3 && s8 == s4) || (s7 == s4 && s8 == s3))
                    continue;
                  if (s8 == s1) continue;
                  G6 = G5 + context.C(s7, s8);
                  Gain = G6 - context.C(s8, s1);
                  if (Gain > 0) {
                    if (Case8 <= 15) {
                      Make4OptMove(s1, s2, s3, s4, s5, s6, s7, s8, Case8);
                      if (Improvement(&Gain, s1, s2)) return Gain;
                    } else if (Gain > Gain6) {
                      Gain = BridgeGain(s1, s2, s3, s4, s5, s6, s7, s8, Case6,
                                        Gain);
                      if (Gain > 0) return Gain;
                    }
                  }
