...
inline int dis(Node *a, Node *b) {
  return a == b ? 0 : context.D(a, b) - a->Pi - b->Pi;
}
...
inline int unif(int low, int high) { return low + Random() % (high - low + 1); }

inline void shuffle(int N, int *path) {
...
void POpMUSICThreeOpt::path_threeOpt(int N, int **D, int *best_sol,
  ...
  *best_cost = bestTourLength;
  for (i = 0; i < n; i++) pos[tour[i] = bestTour[i]] = i;
  reversed = next(0) == N;
  for (i = 0, j = 0; j < n; i = NEXT(i), j++) best_sol[j] = i;
  free(tour);
  free(pos);
  free(bestTour);
  free(neighbors);
...
int POpMUSICThreeOpt::fixed(int a, int b) {
  return (a == 0 && b == n - 1) || (a == n - 1 && b == 0);
}

int POpMUSICThreeOpt::prev(int v) {
  return tour[pos[v] > 0 ? pos[v] - 1 : n - 1];
}

int POpMUSICThreeOpt::next(int v) {
  return tour[pos[v] < n - 1 ? pos[v] + 1 : 0];
}

int POpMUSICThreeOpt::between(int v1, int v2, int v3) {
  int a = pos[v1], b = pos[v2], c = pos[v3];
  return a <= c ? b >= a && b <= c : b <= c || b >= a;
}

int POpMUSICThreeOpt::PREV(int v) { return reversed ? next(v) : prev(v); }

int POpMUSICThreeOpt::NEXT(int v) { return reversed ? prev(v) : next(v); }

int POpMUSICThreeOpt::BETWEEN(int v1, int v2, int v3) {
  return !reversed ? between(v1, v2, v3) : between(v3, v2, v1);
}

void POpMUSICThreeOpt::flip(int from, int to) {
  int i, j, size, tmp;
    ...
    for (b = 0; b < n; b++) {
      ...
      for (xa = 1; xa <= 2; xa++, reversed = !reversed) {
        ...
        for (i = 0; i < neighbors[b]; i++) {
          c = neighbor[b][i];
          if (c == prev(b) || c == next(b)) continue;
          g1 = g0 - dist[b][c];
          if (g1 <= 0) break;
          for (xc = 1; xc <= 2; xc++) {
            d = xc == 1 ? PREV(c) : NEXT(c);
            if (d == a || fixed(c, d)) continue;
            g2 = g1 + dist[c][d];
            if (xc == 1) {
              gain = g2 - dist[d][a];
              if (gain > 0) {
            ...
            for (j = 0; j < neighbors[d]; j++) {
              ...
              for (xe = 1; xe <= xc; xe++) {
                if (xc == 1)
                  f = BETWEEN(b, e, c) ? NEXT(e) : PREV(e);
                else
                  f = xe == 1 ? PREV(e) : NEXT(e);
                if (f == a || fixed(e, f)) continue;
...
void POpMUSICThreeOpt::createNeighbors(void) {
  int i, j, k, d;
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      if (i == j) continue;
      d = dist[i][j];
      k = neighbors[i] < max_neighbors ? neighbors[i]++ : max_neighbors;
...
int POpMUSICThreeOpt::legal(int r, int i, int t[4]) {
  while (--i >= 0)
    if (r == t[i]) return 0;
  return !fixed(r, next(r));
}

int POpMUSICThreeOpt::select_t(int i, int t[4]) {
  int r, r0;
  r = r0 = unif(0, n - 1);
  while (!legal(r, i, t)) {
    if (++r == n) r = 0;
...
void POpMUSICThreeOpt::doubleBridgeKick(void) {
  ...
  if (pos[t[0]] > pos[t[1]]) swap(t, t + 1);
  if (pos[t[2]] > pos[t[3]]) swap(t + 2, t + 3);
  if (pos[t[0]] > pos[t[2]]) swap(t, t + 2);
  if (pos[t[1]] > pos[t[3]]) swap(t + 1, t + 3);
  if (pos[t[1]] > pos[t[2]]) swap(t + 1, t + 2);
  a = t[0];
  b = next(a);
  c = t[2];
  d = next(c);
  ...
  tourLength -= dist[a][b] - dist[b][c] + dist[c][d] - dist[d][a] + dist[e][f] -
                dist[f][g] + dist[g][h] - dist[h][e];
}
...
void POpMUSICCandicateSetCreator::CreateCandidateSet(size_t K) {
    ...
    param.initial_tour_algorithm = WALK;
    context.Trial = 1;
    ...
    for (no_res = 1; no_res <= solutions_; no_res++) {
        ...
        startTime = GetTime();
        build_path(n, solution, sample_size_);
        solution[n] = solution[0];
        node[n] = node[solution[0]];
        cost = length_path(n, solution);
        std::stringstream ss;
        ss << no_res << ": Initial cost: " << cost;
        if (context.Optimum != std::numeric_limits<GainType>::min() &&
            context.Optimum != 0)
          ss << ", Gap = " << std::fixed << std::setprecision(2)
    ...
    ss << "Cost: min = " << costMin << ", Cost.avg = " << (double) costSum / solutions_ << ", max = " << costMax;
    if (context.Optimum != std::numeric_limits<GainType>::min() &&
        context.Optimum != 0) {
      ss << ", Gap.min = "
         << 100.0 * (costMin - context.Optimum) / context.Optimum << "%";
      ss << ", Gap.avg = "
         << 100.0 * ((double)costSum / solutions_ - context.Optimum) /
                context.Optimum
         << "%";
      ss << ", Gap.max = "
...
GainType POpMUSICCandicateSetCreator::length_path(int n, int *path) {
  Node *a, *b;
  GainType length = 0;
  int i;
...
void POpMUSICCandicateSetCreator::optimize_path(int n, int *path) {
  int i, j;
  int *order, **d;
  GainType length;
  Node *a, *b;
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = i + 1; j <= n; j++) {
      b = node_path[j];
      d[i][j] = d[j][i] = a != b ? dis(a, b) : INT_MAX;
    }
  }
  d[0][n] = d[n][0] = 0;
  length = 0;
  for (i = 0; i < n; i++) length += d[i][i + 1];
  three_opt.path_threeOpt(n, d, order, &length);
  for (i = 0; i <= n; i++) free(d[i]);
  free(d);
  for (i = 0; i <= n; i++) order[i] = path[order[i]];
  for (i = 0; i <= n; i++) path[i] = order[i];
  free(order);
}

/*********** Build recursively a path between path[0] and path[n] *************/
void POpMUSICCandicateSetCreator::build_path(int n, int *path, int nb_clust) {
  ...
  for (i = 0; i <= n; i++) tmp_path[i] = path[i];

  /* Let tmp_path[1] be the closest city to path[0] */
  dmin = dis(node[tmp_path[1]], node[path[0]]);
  closest = 1;
  for (i = 2; i < n; i++) {
    if ((d = dis(node[tmp_path[i]], node[path[0]])) < dmin) {
      dmin = d;
      closest = i;
    }
  }
  ...
  dmin = dis(node[tmp_path[2]], node[path[n]]);
  closest = 2;
  for (i = 3; i < n; i++) {
    if ((d = dis(node[tmp_path[i]], node[path[n]])) < dmin) {
      dmin = d;
      closest = i;
    }
  }
  ...
  for (i = 3; i <= nb_clust; i++) swap(tmp_path + i, tmp_path + unif(i, n - 1));
  swap(tmp_path + 2, tmp_path + nb_clust);
  sample = (int *)malloc((nb_clust + 2) * sizeof(int));
  for (i = 0; i <= nb_clust; i++) sample[i] = tmp_path[i];
  ...
  for (i = 1; i < n; i++) {
    ...
    for (j = 1; j <= nb_clust; j++) {
      ...
      if ((d = dis(node[path[i]], node[sample[j]])) < dmin) {
        dmin = d;
        closest = j;
      }
    }
  ...
  for (i = 1; i < n; i++) start_clust[assignment[i]]++;
  for (i = 1; i <= nb_clust; i++) start_clust[i] += start_clust[i - 1];

  /* Clusters are stored in tmp_path in the order given by sample */
  assigned = (int *)calloc(nb_clust + 1, sizeof(int));
  for (i = 1; i < n; i++)
    tmp_path[start_clust[assignment[i] - 1] + assigned[assignment[i]]++] =
        path[i];

  /* Reorder original path */
  for (i = 1; i < n; i++) path[i] = tmp_path[i - 1];
  free(tmp_path);
  free(sample);
  free(assigned);
  free(assignment);
...
void POpMUSICCandicateSetCreator::reverse(int *path, int i, int j) {
  while (i < j) swap(path + i++, path + j--);
}

void POpMUSICCandicateSetCreator::circular_right_shift(int n, int *path,
                                                       int positions) {
...
void POpMUSICCandicateSetCreator::fast_POPMUSIC(int n, int *path, int R) {
  int scan, i;
  ...
  for (scan = 1; scan <= 2; scan++) {
    ...
    for (i = 0; i < n / R; i++) optimize_path(R, path + R * i);
