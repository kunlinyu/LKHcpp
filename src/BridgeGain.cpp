...
GainType BridgeGain(Node* s1, Node* s2, Node* s3, Node* s4, Node* s5, Node* s6,
  ...
  for (t1 = u2; t1 != u3; t1 = t2) {
    ...
    for (const auto& c2 : t2->candidates) {
      Nt2 = c2;
      t3 = Nt2->To;
      if (t3 == t2->Prd || t3 == t2->Suc || BETWEEN(u2, t3, u3)) continue;
      G1 = G0 - Nt2->Cost;
      if (++Breadth2 > param.max_breadth) break;
      /* Choose t4 as one of t3's two neighbors on the tour */
      for (X4 = 1; X4 <= 2; X4++) {
        t4 = X4 == 1 ? SUC(t3) : PRD(t3);
        if (t4 == t2 || (t3 == s1 && t4 == s2) || (t3 == s2 && t4 == s1) ||
            (t3 == s3 && t4 == s4) || (t3 == s4 && t4 == s3) ||
            (t3 == s5 && t4 == s6) || (t3 == s6 && t4 == s5) ||
            (t3 == s7 && t4 == s8) || (t3 == s8 && t4 == s7))
          continue;
        G2 = G1 + context.C(t3, t4);
        /* Test if an improvement can be obtained */
        if (t4 != t1 && (t4 != s2 || t1 != s3) && (t4 != s3 || t1 != s2) &&
            (t4 != s4 || t1 != s5) && (t4 != s5 || t1 != s4) &&
            (t4 != s6 || t1 != s7) && (t4 != s7 || t1 != s6) &&
            (t4 != s8 || t1 != s1) && (t4 != s1 || t1 != s8) &&
            (s8 || ((t4 != s6 || t1 != s1) && (t4 != s1 || t1 != s6)))) {
          ...
          if (Gain > 0) {
            switch (Case6) {
              case 0:
                ...
                break;
              case 3:
                if ((X4 == 1) == (!BETWEEN(s2, t1, s6) && !BETWEEN(s2, t3, s6)))
                  Swap3(s1, s2, s3, t1, t2, t3, s5, s6, s1);
                else
                  Swap4(s1, s2, s3, t1, t2, t4, s5, s6, s1, t2, t4, t1);
                if (s8) Swap1(s7, s8, s1);
                break;
              case 4:
                if ((X4 == 1) == (!BETWEEN(s3, t1, s5) && !BETWEEN(s3, t3, s5)))
                  Swap3(s1, s2, s3, t1, t2, t3, s5, s6, s1);
                else
                  Swap4(s1, s2, s3, t1, t2, t4, s5, s6, s1, t2, t4, t1);
                if (s8) Swap1(s7, s8, s1);
                break;
              case 7:
                if ((X4 == 1) == (!BETWEEN(s4, t1, s6) && !BETWEEN(s4, t3, s6)))
                  Swap3(s5, s6, s1, t1, t2, t3, s3, s4, s5);
                else
                  Swap4(s5, s6, s1, t1, t2, t4, s3, s4, s5, t2, t4, t1);
                if (s8) Swap1(s7, s8, s1);
                break;
            }
            if (Improvement(&Gain, s1, s2)) return Gain;
          }
        }
        ...
        for (const auto& c4 : t4->candidates) {
          Nt4 = c4;
          t5 = Nt4->To;
          if (t5 == t4->Prd || t5 == t4->Suc || t5 == t1 || t5 == t2) continue;
          /* Choose t6 as one of t5's two neighbors on the tour.
             Only one choice! */
          t6 = X4 == 1 || BETWEEN(u2, t5, u3) ? PRD(t5) : SUC(t5);
          if ((t5 == s1 && t6 == s2) || (t5 == s2 && t6 == s1) ||
              (t5 == s3 && t6 == s4) || (t5 == s4 && t6 == s3))
            continue;
          G3 = G2 - Nt4->Cost;
          G4 = G3 + context.C(t5, t6);
          if (t6 != t1 && (Gain = G4 - context.C(t6, t1)) > 0) {
            if (X4 == 1)
              Swap4(s1, s2, s4, t3, t4, t1, s1, s3, s2, t5, t6, t1);
            else
              Swap3(t1, t2, t3, s1, s2, s3, t5, t6, t1);
            if (Improvement(&Gain, s1, s2)) return Gain;
          }
          if (++Breadth4 > param.max_breadth) break;
          Breadth6 = 0;
          ...
          for (const auto& c6 : t6->candidates) {
            Nt6 = c6;
            t7 = Nt6->To;
            if (t7 == t6->Prd || t7 == t6->Suc) continue;
            /* Choose t8 as one of t7's two neighbors on the tour.
               Only one choice! */
            if (X4 == 1)
              t8 = (BETWEEN(u2, t5, t1) ? BETWEEN(t5, t7, t1)
                    : BETWEEN(t2, t5, u3)
                        ? BETWEEN(u2, t7, t1) || BETWEEN(t5, t7, u3)
                    : BETWEEN(SUC(u3), t5, t3)
                        ? BETWEEN(u2, t7, u3) || BETWEEN(t5, t7, t3)
                        : !BETWEEN(t4, t7, t6))
                       ? PRD(t7)
                       : SUC(t7);
            else
              t8 = (BETWEEN(u2, t5, t1)
                        ? !BETWEEN(u2, t7, t6) && !BETWEEN(t2, t7, u3)
                    : BETWEEN(t2, t5, u3) ? !BETWEEN(t2, t7, t6)
                    : BETWEEN(SUC(u3), t5, t4)
                        ? !BETWEEN(SUC(u3), t7, t5) && !BETWEEN(t3, t7, PRD(u2))
                        : !BETWEEN(t3, t7, t5))
                       ? PRD(t7)
                       : SUC(t7);
            if (t8 == t1 || (t7 == t1 && t8 == t2) || (t7 == t3 && t8 == t4) ||
                (t7 == t4 && t8 == t3) || (t7 == s1 && t8 == s2) ||
                (t7 == s2 && t8 == s1) || (t7 == s3 && t8 == s4) ||
                (t7 == s4 && t8 == s3))
              continue;
            if (t8 == t1) continue;
            G5 = G4 - Nt6->Cost;
            ...
            if (++Breadth6 > param.max_breadth) break;
          }
        }
      }
    }
